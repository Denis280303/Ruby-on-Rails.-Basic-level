module Validation
  ...
  module InstanceMethods
    ...
    def presence(value, _arg)
      raise 'Значення не має бути порожнім' if value.nil? value.empty?
    end

    def format(value, format)
      raise 'Значення не відповідає формату' if value !~ format
    end

    def type(value, attribute_class)
      raise "Об'єкт не відповідає класу" unless attribute_class.include? value
    end
